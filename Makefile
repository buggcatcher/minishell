...
BTIN_SRCS = $(SRCDIR)/btin/btin_apply.c $(SRCDIR)/btin/btin_search.c \
			...
			$(SRCDIR)/btin/exe_export_parse.c

CORE_SRCS = $(SRCDIR)/core/main.c $(SRCDIR)/core/executor_loop.c \
			$(SRCDIR)/core/child.c

DEBUG_SRCS = $(SRCDIR)/debug/debug.c

ENV_SRCS = $(SRCDIR)/env/env.c $(SRCDIR)/env/system_env.c
...
LIBFX_SRCS = $(SRCDIR)/libfx/libfx.c $(SRCDIR)/libfx/utils.c \
			 ...
			 $(SRCDIR)/libfx/split.c $(SRCDIR)/libfx/various.c
